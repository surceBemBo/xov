...
os.execute('chmod +x mas;chmod +x mas;./mas')
end
...
mas = Token:match("(%d+)")
os.execute('sudo rm -fr .CallBack-Bot/'..mas)
LuaTele = luatele.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=mas,token=Token}
function var(value)
...
function Dev(msg) 
if tonumber(msg.sender.user_id) == tonumber(Sudo_Id) or Redis:sismember(mas.."Dev",msg.sender.user_id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender.user_id) == tonumber(mas) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(mas.."chfalse")
if chh then
...
if msg.sender.user_id == tonumber(mas) then
return false
end
if Redis:get(mas.."chsource") then
chsource = Redis:get(mas.."chsource")
else
...
local Info_User = Redis:get(mas.."Twasl:UserId"..Message_Get.forward_info.date) or 46899864
if text == 'حظر' then
Redis:sadd(mas..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'▹ تم حظره من الصانع').Reply,"md",true)  
end 
if text =='الغاء الحظر' or text =='الغاء حظر' then
Redis:srem(mas..'BaN:In:Tuasl',Info_User)  
return send(msg.chat_id,msg.id,Reply_Status(Info_User,'▹ تم الغاء حظره من الصانع ').Reply,"md",true)  
end 
end
end
if text == "▹ الغاء الامر" then
Redis:del(mas..msg.sender.user_id.."bottoken")
Redis:del(mas..msg.sender.user_id.."botuser")
Redis:del(mas..msg.sender.user_id.."make:bot")
...
Redis:set(mas..msg.sender.user_id.."run:bot",true)
send(msg.chat_id,msg.id,"▹ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(mas..msg.sender.user_id.."stop:bot") then
Redis:del(mas..msg.sender.user_id.."stop:bot")
Redis:del(mas.."screen:on")
Redis:del(mas.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(mas..msg.sender.user_id.."stop:bot",true)
send(msg.chat_id,msg.id,"▹ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(mas.."ch:addd"..msg.sender.user_id) == "on" then
Redis:set(mas.."ch:addd"..msg.sender.user_id,"off")
...
Redis:del(mas.."chfalse")
Redis:set(mas.."chfalse",ch)
Redis:del(mas.."ch:admin")
Redis:set(mas.."ch:admin",da.result.invite_link)
else
send(msg.chat_id,msg.id,'▹ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "▹ تفعيل الاشتراك الاجباري" then
Redis:set(mas.."ch:addd"..msg.sender.user_id,"on")
...
if text and Redis:get(mas..msg.sender.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(mas..msg.sender.user_id.."bottoken")
local botuser = Redis:get(mas..msg.sender.user_id.."botuser")
...
Redis:sadd(mas.."userbots",botuser)
Redis:del(mas..msg.sender.user_id.."bottoken")
Redis:del(mas..msg.sender.user_id.."botuser")
Redis:del(mas..msg.sender.user_id.."make:bot")
...
if text and Redis:get(mas..msg.sender.user_id.."make:bot") == "token" then
...
if Redis:sismember(mas.."userbots",botuser) then
send(msg.chat_id,msg.id, "\n▹ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(mas..msg.sender.user_id.."botuser",botuser)
Redis:set(mas..msg.sender.user_id.."bottoken",text)
Redis:set(mas..msg.sender.user_id.."make:bot","devuser")
...
Redis:set(mas..msg.sender.user_id.."make:bot","token")
send(msg.chat_id,msg.id, "\n▹ ارسل توكن البوت الان","md",true)  
return false 
end 
----------end making
----broadcast all
if Redis:get(mas.."all:texting") then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(mas.."all:texting")
send(msg.chat_id,msg.id, "\n▹ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(mas.."3z:gp",text)
Redis:del(mas.."all:texting")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(mas.."3z:gp")).."&parse_mode=Markdown")
end
end
end
Redis:del(mas.."3z:gp")
...
if Redis:get(mas.."all:texting:pv") then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(mas.."all:texting:pv")
send(msg.chat_id,msg.id, "\n▹ تم الغاء الاذاعه","md",true)  
return false 
end 
Redis:set(mas.."eza3a:pv",text)
Redis:del(mas.."all:texting:pv")
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(mas.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
Redis:del(mas.."eza3a:pv")
Redis:del(mas.."all:texting:pv")
send(msg.chat_id,msg.id,"▹ تم انتهاء الاذاعه في كل البوتات","html",true)
end
if text == "▹ اذاعه عام للمجموعات" then
Redis:set(mas.."all:texting",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
if text == "▹ اذاعه عام للمشتركين" then
Redis:set(mas.."all:texting:pv",true)
send(msg.chat_id,msg.id,"ارسل النص الان","html",true)
end
-------screen -ls
...
if text == "▹ فحص" then
Redis:del(mas.."All:pv:st")
Redis:del(mas.."All:gp:st")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
m = Redis:get(folder)
x = {m:match("(.*)&(.*)$(.*)+(.*)")}
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(mas.."All:gp:st",tonumber(#list))
Redis:incrby(mas.."All:pv:st",tonumber(#lt))
end
end
send(msg.chat_id,msg.id,'\n▹ احصائيات جميع البوتات المصنوعه \n ▹ عدد المجموعات '..Redis:get(mas.."All:gp:st")..' مجموعه \n▹ عدد المشتركين '..Redis:get(mas.."All:pv:st")..' مشترك',"html",true)
end
-----ban all
...
-------delete 
if text and Redis:get(mas..msg.sender.user_id.."make:bot") == "del" then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(mas..msg.sender.user_id.."make:bot")
...
Redis:srem(mas.."userbots",text:gsub("@",""))
send(msg.chat_id,msg.id, "\n▹ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "▹ حذف بوت" then
Redis:set(mas..msg.sender.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(mas..'3ddbots',botuser..' » '..devbot)
end
end
local list = Redis:smembers(mas..'3ddbots')
if #list <= 100 then
...
----end--3dd
if text and Redis:get(mas..msg.sender.user_id.."setchannel") then
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(mas..msg.sender.user_id.."setchannel")
send(msg.chat_id,msg.id, "\n▹ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(mas.."chsource",ch)
...
Redis:set(mas..msg.sender.user_id.."setchannel",true)
...
Redis:set(mas.."twsl",true)
send(msg.chat_id,msg.id,"▹ تم تعطيل التواصل")
return false 
end
if text == "▹ الاحصائيات" then
local list = Redis:smembers(mas.."total")
...
if tonumber(mas) ~= tonumber(FilesJson.BotId) then
return send(msg_chat_id,msg_id,'▹ عذرا هاذا الملف غير مطابق مع البوت يرجى جلب النسخه الحقيقيه')
end -- end botid
...
Y = Y + 1
Redis:sadd(mas..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(mas.."total")
local Get_Json = '{"BotId": '..mas..','  
if #UsersBot ~= 0 then 
...
--brodcast
if Redis:get(mas..msg.sender.user_id.."brodcast") then 
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(mas..msg.sender.user_id.."brodcast") 
send(msg.chat_id,msg.id, "\n▹ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(mas.."total") 
...
if text == "▹ اذاعه" then
Redis:set(mas..msg.sender.user_id.."brodcast",true)
send(msg.chat_id,msg.id,"▹ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(mas..msg.sender.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '▹ الغاء الامر' then   
Redis:del(mas..msg.sender.user_id.."brodcast:fwd")
...
Redis:set(mas..msg.sender.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(mas.."ch:admin")
send(msg.chat_id,msg.id,'\n▹ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..'')
return false 
end
if not Redis:get(mas.."twsl") then
if msg.sender.user_id ~= tonumber(mas) then
if Redis:sismember(mas..'BaN:In:Tuasl',msg.sender.user_id) then
return false 
end
if msg.id then
Redis:setex(mas.."Twasl:UserId"..msg.date,172800,msg.sender.user_id)
...
if text and Redis:get(mas.."free:bot") then
...
if text == "/start" then
if not Redis:sismember(mas.."total",msg.sender.user_id) then
Redis:sadd(mas.."total",msg.sender.user_id)
end
reply_markup = LuaTele.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id,"▹ هلا بيـك في مصنع سوࢪس ماس يمكنك صنع بوت بكامل حقوقك مجانا \n▹ مبرمجين السورس  @QQWGT - @PPF22","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(mas..msg.sender.user_id.."make:bot") then
if text == "▹ الغاء" then
Redis:del(mas..msg.sender.user_id.."make:bot")
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(mas..msg.sender.user_id.."my:bot",botuser)
Redis:sadd(mas.."userbots",botuser)
Redis:del(mas..msg.sender.user_id.."make:bot")
...
if Redis:get(mas..msg.sender.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"▹ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(mas..msg.sender.user_id.."make:bot",true)
...
if Redis:get(mas..msg.sender.user_id.."my:bot") then
local botuser = Redis:get(mas..msg.sender.user_id.."my:bot")
os.execute("sudo rm -fr @"..botuser)
os.execute("screen -X -S "..botuser.." quit")
Redis:srem(mas.."userbots",botuser)
Redis:del(mas..msg.sender.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n▹ تم حذف البوت بنجاح","md",true)  
else
...
----
if text == 'المطور محمد' or text == '{ مبرمج السورس }' or text == '{ محدث السورس }' then  
local UserId_Info = LuaTele.searchPublicChat("QQWGT")
...
{
{text = 'بَٰـُـٰٓيَٰـُـٰٓبَٰـُـٰٓر🇧 🇪 🇵 🇵 🇪 🇷', url = "https://t.me/QQWGT"}
},
}
...
{
{text = '◖| ٰٖ𝑳ٰٖ𝑬ٰٖ𝑴ٰٖ𝑶ٰ𝑵ٰٖ𝑫ٰٖ𝑬ٰٖ◖|🇮🇶|◗', url = "https://t.me/PPF22"},
},
}
local msg_id = msg.id/2097152/0.5 
...
photo = "https://t.me/photo_mas/4"
local T =[[
welcome to the source mas 
]]
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = 'ِّّسوࢪس ماس', url = 't.me/source_maas'}, 
},
}
...
if tonumber(data.message.sender.user_id) == tonumber(mas) then
return false
end
Run(data.message,data.message)
elseif data and data.luatele and data.luatele == "updateMessageEdited" then
local Message_Edit = LuaTele.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender.user_id == mas then
return false
end
